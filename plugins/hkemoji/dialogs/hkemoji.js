CKEDITOR.dialog.add( 'hkemojiDialog', function( editor ) {
	...
	var buildHtml = function(group) {
		...
		for (var shortcode in list) {
			...
			var shortname = shortcode.substr(0,shortcode.lastIndexOf("."));

			html.push(
				'<td class="cke_centered" style="vertical-align: middle;" role="presentation">' +
				'<img class="hkemoji" cke_src="'+ CKEDITOR.tools.htmlEncode(urlimage) + '" alt="'+shortname+'" src="'+ urlimage + '" href="javascript:void(0)" onkeydown="CKEDITOR.tools.callFunction( ', onKeydown, ', event, this );"/>', '</td>'
			);
	...
	return {
		...
		contents: [
			{
				id: 'tab-onion',
				label: editor.config.hkemoji.tabs.ac,
				elements: [
					emojis('ac')
				]
			}, {
				id: 'tab-red',
				label: editor.config.hkemoji.tabs.ac2,
				elements: [
					emojis('ac2')
				]
			}
